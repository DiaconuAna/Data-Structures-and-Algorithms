...
void testRemove() {
	...
	//printf("\ni= %d, nr = %d",0, b.nrOccurrences(0));
	assert(b.size() == 800);
	for (int i = -100; i < 100; i++) {
	    //printf("\ni= %d, nr = %d",i, b.nrOccurrences(i));
...
void testIterator() { 
	...
	for (unsigned int i = 0; i < elements.size(); i++) { 
       ...
       // printf("Last elem %d its occurrence %d\n", lastElem, b4.nrOccurrences(lastElem));
	...
	for (unsigned int i = 0; i < elements2.size(); i++) { 
		...
		//cout<<"Number of ocurrences "<< b5.nrOccurrences(firstElem)<<" "<< firstElem << endl;
		assert(b5.search(firstElem) == true);
		//b5.contents();
		b5.remove(firstElem);
		//cout<<"After removal of "<<firstElem<<endl;
		//b5.contents();
	}
}
