...
void testIteratorSteps(SortedMultiMap& m) {
	...
	for (int i = 0; i < count / 2; i++) {
	    ...
	    //cout<<smmi.getCurrent().first<< " "<<smmi.getCurrent().second<<std::endl;
		smmi.next();
	...
	//	std::cout<<smmi.getCurrent().first<<" "<<smmi.getCurrent().second<<std::endl;
		//if(c==302){
		//    smmi.valid();
	//	}
		smmi.next();
	}
	//std::cout<<c<<" "<<m.size()<<std::endl;
	assert(c == m.size());
}
